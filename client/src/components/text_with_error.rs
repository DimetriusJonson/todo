...
#[component]
pub fn TextWithError(props: &Props) -> Html {
    ...
    let mut classes = "input".to_owned();
    if !props.error.is_empty() {
        classes.push_str(" is-danger");
    }

    html! {
        <>
            <div class="control">
                <input ref={input_ref} class={classes} type={props.input_type.to_string()} id={props.name.to_owned()} name={props.name.to_owned()}
